...
@interface SafariFullUrlController : PSListController {
	UILabel* _label;
	UILabel* underLabel;
}
- (void)HeaderCell;
...
@implementation SafariFullUrlController
- (id)specifiers {
	if (!_specifiers) {
		...
		spec = [PSSpecifier preferenceSpecifierNamed:@"Remove Prefix On URL"
                                                  target:self
											         set:@selector(setPreferenceValue:specifier:)
											         get:@selector(readPreferenceValue:)
                                                  detail:Nil
											        cell:PSSwitchCell
											        edit:Nil];
		[spec setProperty:@"RemovePrefix" forKey:@"key"];
        ...
        [specifiers addObject:spec];
		spec = [PSSpecifier emptyGroupSpecifier];
        [spec setProperty:@"Safari Full Url © 2016 julioverne" forKey:@"footerText"];
        [specifiers addObject:spec];
...
- (void)love
{
	...
	[twitter setInitialText:@"#SafariFullUrl by @ijulioverne is cool!"];
	if (twitter != nil) {
...
- (void)reset
{
	...
	notify_post("com.julioverne.safarifullurl/Settings");
}
...
- (void)HeaderCell
{
	...
	CGRect frame = CGRectMake(0, 20, width, 60);
		...
		[_label setText:@"Safari Full Url"];
		...
		[underLabel setText:@"Full URL In Safari"];
